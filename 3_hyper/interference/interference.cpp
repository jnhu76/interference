...
using namespace std;
// -------------------------------------------------------------------------------------
ub8 PAGE_FLUSH_PAGE_COUNT = 58000;
ub8 LOG_BYTE_COUNT = 2e8;
ub8 SEQ_READER_BYTE_COUNT = 2e9;
ub8 RND_READER_BYTE_COUNT = 2e8;
// -------------------------------------------------------------------------------------
...
ub4 BASE_TEST = 1;
bool arachne_flags[6] = {true, true, true, true, true, true};
ub4 MIN_CORES = 0;
// -------------------------------------------------------------------------------------
...
vector<Arachne::ThreadId> all_arachne_threads;
// -------------------------------------------------------------------------------------
// Common config
string NVM_PATH = "";
...
void CreateAllThreads()
{
   ...
   for (ub4 tid = 0; tid<SEQ_RAM_THREADS; tid++) {
      seq_ram_readers.emplace_back(make_unique<SequentialReader>(NVM_PATH + "/seq_ram_reader_" + to_string(tid), SEQ_READER_BYTE_COUNT, true, tid, CONFIG_STRING, arachne_flags[0]));
      if (!arachne_flags[0]) {
         all_threads.emplace_back(make_unique<thread>([&, tid]() {
            seq_ram_readers[tid]->Run();
         }));
      } else {
         Arachne::ThreadId aid = Arachne::createThreadWithClass(1, &SequentialReader::Run, seq_ram_readers[tid].get());
         all_arachne_threads.emplace_back(std::move(aid));
      }
   ...
   for (ub4 tid = 0; tid<SEQ_NVM_THREADS; tid++) {
      seq_nvm_readers.emplace_back(make_unique<SequentialReader>(NVM_PATH + "/seq_nvm_reader_" + to_string(tid), SEQ_READER_BYTE_COUNT, false, tid, CONFIG_STRING, arachne_flags[1]));
      if (!arachne_flags[1]) {
         all_threads.emplace_back(make_unique<thread>([&, tid]() {
            seq_nvm_readers[tid]->Run();
         }));
      } else {
         Arachne::ThreadId aid = Arachne::createThreadWithClass(1, &SequentialReader::Run, seq_nvm_readers[tid].get());
         all_arachne_threads.emplace_back(std::move(aid));
      }
   ...
   for (ub4 tid = 0; tid<RND_RAM_THREADS; tid++) {
      rnd_ram_readers.emplace_back(make_unique<RandomReader>(NVM_PATH + "/rnd_ram_reader_" + to_string(tid), RND_READER_BYTE_COUNT, true, tid, CONFIG_STRING, arachne_flags[2]));
      if (!arachne_flags[2]) {
      all_threads.emplace_back(make_unique<thread>([&, tid]() {
         rnd_ram_readers[tid]->Run();
      }));
      } else {      
         Arachne::ThreadId aid = Arachne::createThreadWithClass(1, &RandomReader::Run, rnd_ram_readers[tid].get());
         all_arachne_threads.emplace_back(std::move(aid));
      }
   ...
   for (ub4 tid = 0; tid<RND_NVM_THREADS; tid++) {
      rnd_nvm_readers.emplace_back(make_unique<RandomReader>(NVM_PATH + "/rnd_nvm_reader_" + to_string(tid), RND_READER_BYTE_COUNT, false, tid, CONFIG_STRING, arachne_flags[3]));
      if (!arachne_flags[3]) {
         all_threads.emplace_back(make_unique<thread>([&, tid]() {
            rnd_nvm_readers[tid]->Run();
         }));
      } else {
         Arachne::ThreadId aid = Arachne::createThreadWithClass(1, &RandomReader::Run, rnd_nvm_readers[tid].get());
         all_arachne_threads.emplace_back(std::move(aid));
      }
   ...
   for (ub4 tid = 0; tid<LOG_NVM_THREADS; tid++) {
      log_writers.emplace_back(make_unique<LogWriter>(NVM_PATH + "/log_writer_" + to_string(tid), LOG_BYTE_COUNT, tid, CONFIG_STRING, arachne_flags[4]));
      if (!arachne_flags[4]) {
         all_threads.emplace_back(make_unique<thread>([&, tid]() {
            log_writers[tid]->Run();
         }));
      } else {
         Arachne::ThreadId aid = Arachne::createThreadWithClass(1, &LogWriter::Run, log_writers[tid].get());
         all_arachne_threads.emplace_back(std::move(aid));
      }
   ...
   for (ub4 tid = 0; tid<PAGE_NVM_THREADS; tid++) {
      page_flushers.emplace_back(make_unique<PageFlusher>(NVM_PATH + "/page_flush_" + to_string(tid), PAGE_FLUSH_PAGE_COUNT, tid, CONFIG_STRING, arachne_flags[5]));
      if (!arachne_flags[5]) {
         all_threads.emplace_back(make_unique<thread>([&, tid]() {
            page_flushers[tid]->Run();
         }));      
      } else {
         Arachne::ThreadId aid = Arachne::createThreadWithClass(1, &PageFlusher::Run, page_flushers[tid].get());
         all_arachne_threads.emplace_back(std::move(aid));
      }
...
void WaitForAllToDie()
{
   ...
   for (auto iter: all_arachne_threads) {
      Arachne::join(iter);
   }
}
// -------------------------------------------------------------------------------------
...
{
   if (argc != 9) {
      cout << "usage: " << argv[0] << " SEQ_RAM SEQ_NVM RND_RAM RND_NVM LOG_NVM PAGE_NVM path BASE" << endl;
      throw;
   }
   ...
   NVM_PATH = argv[7];
   BASE_TEST = stof(argv[8]) - 1;  // 1-6 => 0-5
