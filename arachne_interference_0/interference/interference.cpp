...
void CreateAllThreads()
{
   ...
   for (ub4 tid = 0; tid<SEQ_RAM_THREADS; tid++) {
      ...
      // }));
      Arachne::ThreadId aid = Arachne::createThread(&SequentialReader::Run_helper, (void *)seq_ram_readers[tid].get());
   ...
   for (ub4 tid = 0; tid<SEQ_NVM_THREADS; tid++) {
      ...
      // }));
      Arachne::ThreadId aid = Arachne::createThread(&SequentialReader::Run_helper, (void *)seq_nvm_readers[tid].get());
      all_threads.emplace_back(std::move(aid));
   ...
   for (ub4 tid = 0; tid<RND_RAM_THREADS; tid++) {
      ...
      // }));
      Arachne::ThreadId aid = Arachne::createThread(&RandomReader::Run_helper, (void *) rnd_ram_readers[tid].get());
      all_threads.emplace_back(std::move(aid));
   ...
   for (ub4 tid = 0; tid<RND_NVM_THREADS; tid++) {
      ...
      // }));
      Arachne::ThreadId aid = Arachne::createThread(&RandomReader::Run_helper, (void *) rnd_nvm_readers[tid].get());
      all_threads.emplace_back(std::move(aid));
   ...
   for (ub4 tid = 0; tid<LOG_NVM_THREADS; tid++) {
      ...
      //    log_writers[tid]->Run();
      // }));
      Arachne::ThreadId aid = Arachne::createThread(&LogWriter::Run_helper, (void *) log_writers[tid].get());
      all_threads.emplace_back(std::move(aid));
   ...
   for (ub4 tid = 0; tid<PAGE_NVM_THREADS; tid++) {
      ...
      // }));
      Arachne::ThreadId aid = Arachne::createThread(&PageFlusher::Run_helper, (void *) page_flushers[tid].get());
      all_threads.emplace_back(std::move(aid));
